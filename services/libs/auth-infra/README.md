...
- **JPA 스캔 범위**: `InfraConfig`는 `com.econo.auth.infra` 패키지만 스캔한다. service-client 도메인 JPA Repository/Entity는 `ServiceClientAutoConfiguration`(`com.econo.auth.client.config`)이 `com.econo.auth.client.adapter.out.persistence`를 직접 스캔하여 별도 등록한다. `InfraConfig`에 service-client 패키지를 추가하면 중복 스캔 충돌이 발생한다.
