...
## 개요
auth-api의 application 계층(`LoginTokenService`, `LoginRedirectResolver`, 두 UseCase 인터페이스)을 새 lib 모듈 `services/libs/login`(패키지 `com.econo.auth.login`)로 추출한다. 토큰 서명/검증을 도메인 포트 `TokenCodec`으로 추상화하여 lib이 `spring-security-oauth2` 의존을 0으로 유지하고, Nimbus 기반 구현체는 auth-api의 `config/security/`가 제공한다. 엔드포인트·요청/응답·DB 스키마는 변경 없는 순수 리팩토링이며, 기존 통합 테스트(`AuthApiIntegrationTest`) green을 회귀 안전망으로 삼는다.
...
- [ ] `application/domain/TokenSpec` record 작성 — 필드: `issuer(String)`, `subject(String)`, `issuedAt(Instant)`, `expiresAt(Instant)`, `claims(Map<String,Object>)`. Spring/JWT 타입 의존 없음
- [ ] `application/domain/DecodedToken` record 작성 — 필드: `subject(String)`, `claims(Map<String,Object>)`. Spring/JWT 타입 의존 없음
- [ ] `application/repository/TokenCodec` 인터페이스 작성 — `String encode(TokenSpec spec)`, `DecodedToken decode(String token)`. 도메인 타입만 사용, Spring 타입 의존 없음
...
- [ ] `application/service/LoginTokenService` 작성 (`@Service`) — `JwtEncoder`/`JwtDecoder` 필드 제거, `TokenCodec` 주입. AT/RT 클레임을 `TokenSpec`으로 조립해 `codec.encode()` 호출. `verifyRefreshTokenAndGetMemberId`는 `codec.decode()` 후 `claims.get("token_type")`이 `"refresh"`가 아니면 `WrongTokenTypeException`, 디코딩 실패(`InvalidTokenException` 전파). `@Value` issuer/expiry 유지
...
- [ ] `config/security/NimbusTokenCodecAdapter` 신설 (`implements TokenCodec`) — `encode(TokenSpec)`: `TokenSpec` → `JwtClaimsSet` + `JwsHeader.RS256` 조립 후 `jwtEncoder.encode()` 호출, token 문자열 반환. `decode(String token)`: `jwtDecoder.decode(token)` 후 `Jwt` → `DecodedToken` 변환. `JwtException` 발생 시 `InvalidTokenException`으로 래핑
...
- [ ] `LoginTokenServiceTest` 재작성 및 이동 — `services/libs/login/src/test/java/com/econo/auth/login/application/service/LoginTokenServiceTest.java`로 이동. `JwtEncoder`/`JwtDecoder` mock 폐기, `TokenCodec` mock 방식으로 재작성. `encode()` 호출 시 `TokenSpec` 검증(AT/RT 클레임 포함 여부, `token_type` 값), `verifyRefreshTokenAndGetMemberId` — `token_type=access` 시 `WrongTokenTypeException`, `decode` 실패 시 `InvalidTokenException` 전파 검증
...
- [ ] `NimbusTokenCodecAdapter` 단위 테스트 신설 (`services/apis/auth-api/src/test/...`) — `encode`가 `JwtEncoder` 호출 후 token 문자열 반환 검증. `decode` 성공 시 `DecodedToken` 반환 검증. `JwtException` 발생 시 `InvalidTokenException`으로 래핑 검증
...
- [ ] `docs/ARCHITECTURE.md` 수정 — "보류 사항(auth-api application 계층 lib 추출 / TokenCodec 포트)" 항목을 "완료" 처리로 갱신 또는 제거
- [ ] `docs/adr/0014-3-layer-dip-architecture.md` 수정 — 보류 사항 두 항목("auth-api application 계층 lib 추출", "토큰 발급 추상화") 완료 처리
- [ ] `services/libs/login/README.md` 작성 — README-GUIDE.md 필수 섹션 준수: Quick Reference(패키지, Gradle 경로), 비즈니스 규칙(TokenCodec 포트 설계 의도, lib에 oauth2 의존 금지 이유), 코드 진입점, 에러 코드(`InvalidTokenException`, `WrongTokenTypeException`), 관련 모듈
- [ ] TokenCodec 추상화 결정 ADR 작성 검토 (선택) — `docs/adr/0015-token-codec-port-abstraction.md`. lib 프레임워크 격리 결정 근거 및 트레이드오프 기술
...
- `services/apis/auth-api/src/main/java/com/econo/auth/api/config/RsaKeyConfig.java` — NimbusTokenCodecAdapter가 jwtEncoder/jwtDecoder 빈을 소비
