...
이메일/비밀번호 기반 가입·로그인을 지원하며, 향후 OIDC IdP 작업 시 이 테이블이 end-user pool의 토대가 된다.
...
- **목적**: ECONO 자체 회원 정보(이메일, 비밀번호 해시, 상태, 이메일 인증 여부)를 저장하는 핵심 테이블
- **연관 todo**:
...
| `email` | `VARCHAR(320)` | Y | | | | RFC 5321 최대 이메일 길이 320자. 소문자 정규화 후 저장 |
...
| `status` | `VARCHAR(20)` | Y | `'ACTIVE'` | | | `MemberStatus` enum 문자열 저장. 현재 `ACTIVE`만 사용, `SUSPENDED`/`DELETED` 예약 |
| `email_verified` | `BOOLEAN` | Y | `FALSE` | | | 이메일 인증 완료 여부. 이메일 인증은 별도 작업 범위지만 컬럼은 선제적으로 포함해 향후 마이그레이션 비용 제거 |
...
- PK: `id`
- 유니크: `uq_members_email` on `(email)` — 이메일은 회원 식별자로 중복 불가, 로그인 조회의 인덱스도 겸함
- CHECK: `chk_members_status` — `status IN ('ACTIVE', 'SUSPENDED', 'DELETED')` — 현재 `ACTIVE`만 사용하되 향후 확장 값을 DB 레벨에서 사전 예약해 유효하지 않은 값 삽입 방지

**인덱스**

- `uq_members_email` — `(email)` UNIQUE — 사유: `POST /api/v1/auth/login` 시 이메일로 단건 조회(`findByEmail`)가 매 요청마다 발생하므로 필수. UNIQUE 제약조건이 내부적으로 B-Tree 인덱스를 생성하므로 별도 인덱스 불필요. Flyway 스크립트에서 인덱스 이름을 명시적으로 지정(`CONSTRAINT uq_members_email UNIQUE (email)`)해 JPA `@Table(uniqueConstraints=...)` 선언과 일치시킨다.

> **추가 인덱스 미생성 근거**
> `status` 컬럼 인덱스: 현재 단일 상태(`ACTIVE`)만 사용하므로 선택도가 낮아 인덱스 효과 없음.
> 향후 상태별 목록 조회 기능이 생기면 그 시점에 추가한다.
> `email_verified` 컬럼 인덱스: boolean 컬럼은 선택도가 극히 낮아 인덱스 불필요.
...
CREATE TABLE members (
  ...
  CONSTRAINT chk_members_status CHECK (status IN ('ACTIVE', 'SUSPENDED', 'DELETED'))
);

COMMENT ON TABLE  members                    IS 'ECONO 자체 회원 정보';
COMMENT ON COLUMN members.id                IS '회원 식별자 (PK, auto-increment)';
COMMENT ON COLUMN members.email             IS '이메일 주소 (소문자 정규화 후 저장)';
COMMENT ON COLUMN members.hashed_password   IS 'BCrypt 단방향 해시값 (평문 저장 금지)';
COMMENT ON COLUMN members.status            IS '회원 상태: ACTIVE | SUSPENDED | DELETED';
COMMENT ON COLUMN members.email_verified    IS '이메일 인증 완료 여부 (기본값 false)';
...
|------|--------|------|----------------|--------|
| 1 | `V1__create_members_table.sql` | `members` 테이블 신규 생성 + UNIQUE 제약 + CHECK 제약 | Flyway 미지원(수동 DROP TABLE 필요) | **낮음** — 신규 테이블이므로 기존 데이터 없음. 운영 락 없음 |

> **단일 단계로 처리 가능한 이유**
> - 신규 테이블 생성이므로 기존 데이터에 영향 없음
> - NOT NULL 컬럼에 모두 기본값이 있거나(`status`, `email_verified`, `created_at`) 애플리케이션이 반드시 제공(`email`, `hashed_password`)하므로 백필 불필요
...
- DB 기본값 존재: `status DEFAULT 'ACTIVE'`, `email_verified DEFAULT FALSE`, `created_at DEFAULT NOW()`
- 애플리케이션 필수 제공: `email`, `hashed_password` — `SignupUseCase`에서 검증 후 삽입
...
- `status` CHECK 제약에 `SUSPENDED`, `DELETED`를 미리 예약해 향후 ALTER TABLE 없이 애플리케이션 코드 변경만으로 상태 확장 가능.
...
- [x] todo의 모든 DB 작업이 변경 사항으로 매핑됨
  ...
  - `uq_members_email` UNIQUE 인덱스 (제약조건 겸용) 설계 완료
...
- [x] 프로젝트의 네이밍/PK/타임스탬프 컨벤션 준수
  - 테이블명 복수형 snake_case (`members`)
  - PK: `BIGINT IDENTITY` (Passport.memberId Long 타입 일치)
  - 타임스탬프: `created_at TIMESTAMPTZ` (JPA AuditingEntityListener 연동)
  - 컬럼명 snake_case (JPA `spring.jpa.hibernate.naming.physical-strategy` 기본값 준수)
