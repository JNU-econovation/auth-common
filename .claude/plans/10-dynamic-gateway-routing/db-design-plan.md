...
1. **보호 경로 가로채기**: `/api/v1/auth/login-hijack` 등 `/api/v1/auth/` 하위 경로는 UNIQUE 제약을 통과하지만 auth-api 핵심 경로를 가로챌 수 있다. `ProtectedPathRegistry` 또는 `gateway.protected-paths` 설정과 비교하여 prefix 일치 여부를 사전에 거부해야 한다.
