...
|---|---|---|---|
| PostgreSQL | RDB | 도입 완료 (`member-auth` 작업) | `member-infra` (`auth-api` 통해 연결) |
| Flyway | DB 마이그레이션 | 도입 완료 (`member-auth` 작업) | `member-infra` |
...
| 사용 모듈 | `member-infra` (JPA Entity), `auth-api` (실행 시 연결) |
...
| 항목 | 값 |
|---|---|
| 위치 | `services/libs/member-infra/src/main/resources/db/migration/` |
...
| 버전 | 파일명 | 도입 작업 | 생성 테이블 |
|---|---|---|---|
| V1 | `V1__create_members_table.sql` | `member-auth` | `members` |
| V2 | `V2__create_sas_tables.sql` | `sas-authorization-server` | `oauth2_registered_client`, `oauth2_authorization`, `oauth2_authorization_consent` |
| V3 | `V3__create_spring_session_tables.sql` | `sas-authorization-server` | `SPRING_SESSION`, `SPRING_SESSION_ATTRIBUTES` |
...
- **공통 설정**: 현재 각 모듈(`member-infra`)에 독립적으로 Testcontainer 설정을 보유한다. 여러 모듈 공유가 필요한 경우 Gradle `testFixtures` 방식 도입을 검토한다.
...
- `services/libs/member-infra/README.md` — `member-infra` 모듈의 인프라 사용 방식
