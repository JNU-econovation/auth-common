...
| Flyway | DB 마이그레이션 | 도입 완료, 이후 전역화 (`centralize-db-migration` 작업) | 독립 (`db/migration`), 운영은 flyway 컨테이너 |
...
| 사용 모듈 | `member` (JPA Entity), `auth-api` (실행 시 연결) |
| JDBC 드라이버 | `org.postgresql:postgresql` (`runtimeOnly`) |
| 마이그레이션 | Flyway. SQL 단일 소스는 레포 루트 `db/migration` (어느 모듈도 소유하지 않음). 상세는 아래 마이그레이션 섹션 참조 |
...
마이그레이션 SQL은 **어느 앱/모듈에도 속하지 않고 레포 루트 `db/migration`에서 독립 관리**한다. 동일한 SQL을 운영·로컬·테스트가 각자의 방식으로 적용한다 (단일 진실 소스). 설계 근거는 [ADR-0014](./adr/0014-flyway-container-managed-migration.md).

| 항목 | 값 |
|---|---|
| SQL 위치 (단일 소스) | 레포 루트 `db/migration/` |
| 파일명 컨벤션 | `V{version}__{description}.sql` (대문자 V, 숫자 버전, 언더스코어 두 개, snake_case 설명) |
| DDL 검증 | `auth-api`는 `spring.jpa.hibernate.ddl-auto=validate` (스키마 수정 없이 일치 여부만 확인) |

### 적용 주체 — 환경별

| 환경 | 적용 방식 |
|---|---|
| 운영 (`docker-compose.yml`) | 전용 **flyway 컨테이너**가 `migrate` 실행 후 종료. SQL을 담은 `auth-migration` 이미지(`db/Dockerfile`, `FROM flyway/flyway`)를 CD가 앱과 동일 태그로 빌드·푸시. `auth-api`는 마이그레이션 완료 후(`service_completed_successfully`) 기동하며, 앱 내장 Flyway는 끔(`spring.flyway.enabled=false`). |
| 로컬 (`docker-compose-local.yml`) | 레포가 있으므로 공식 `flyway/flyway` 이미지 + `./db/migration` 볼륨 마운트로 적용 (이미지 빌드 불필요). |
| 테스트 (CI / Gradle) | docker-compose 미사용. `member`·`auth-api` 빌드가 `processTestResources`로 루트 `db/migration`을 테스트 클래스패스에 복사하고, `flyway-core`를 `testImplementation`으로 둬 Testcontainers DB에 적용. |

> **앱은 더 이상 마이그레이션을 적용하지 않는다.** `auth-api` 운영 설정은 `spring.flyway.enabled=false` + `ddl-auto=validate`로, 스키마 적용은 flyway 컨테이너가, 검증만 앱이 담당한다.
...
| V6 | `V6__add_role_to_members.sql` | `admin-ui-passport-role-auth` | `members.role` 컬럼 추가 |
| V7 | `V7__add_owner_id_to_service_client.sql` | `passport-member-self-registration` | `service_client`에 `owner_id`, `client_secret_hash` 컬럼 추가 |
| V8 | `V8__drop_unused_spring_session_tables.sql` | `centralize-db-migration` 이전 | 미사용 Spring Session 테이블 제거 |
...
- **Flyway**: 루트 `db/migration`을 테스트 클래스패스로 복사(`processTestResources`)하고 `flyway-core`를 `testImplementation`으로 둬, 테스트에서도 운영과 동일한 마이그레이션으로 스키마를 만든다. `@DataJpaTest`는 실제 DB(`replace=NONE`)라 `ddl-auto`가 `none`이므로 스키마 생성을 Flyway에 의존한다.
