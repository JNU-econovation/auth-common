...
마이그레이션 SQL은 **어느 앱/모듈에도 속하지 않고 레포 루트 `db/migration`에서 독립 관리**한다. 동일한 SQL을 운영·로컬·테스트가 각자의 방식으로 적용한다 (단일 진실 소스). 설계 근거는 [ADR-0015](./adr/0015-flyway-container-managed-migration.md).
