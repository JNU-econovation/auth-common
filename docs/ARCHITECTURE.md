...
- **passport**는 다른 마이크로서비스가 Passport를 수신·검증하기 위해 사용하는 공유 라이브러리이다.
...
│   └── libs/                         # 공유 라이브러리
│       ├── member-core/              # 도메인 엔티티, 비즈니스 로직 (Spring 독립)
│       ├── member-infra/             # JPA Repository, Flyway 마이그레이션
│       └── passport/                 # Passport, @PassportAuth (외부 서비스용)
...
```
api-gateway ──→ passport

auth-api ──→ member-core
         ──→ member-infra

member-infra ──→ member-core

member-core ──→ passport

passport (독립, 외부 서비스에 배포)
```
...
| **member-core** | Lib | Member 엔티티, 회원가입 비즈니스 로직, 도메인 규칙. Spring 독립. |
| **member-infra** | Lib | JPA Repository, Flyway 마이그레이션 (members, SAS 3종, Spring Session 2종) |
| **passport** | Lib | Passport 도메인, @PassportAuth 어노테이션, ArgumentResolver. 외부 마이크로서비스가 의존하는 공유 라이브러리 |
...
```
com.econo.auth.passport
├── config/                          # Auto-Configuration
│   └── PassportAutoConfiguration   # WebMvcConfigurer 구현, ArgumentResolver 등록
...
### 웹 계층 (passport annotation / resolver 패키지)
...
### 설정 계층 (passport config 패키지)

- **PassportAutoConfiguration**: `WebMvcConfigurer`를 구현하여 `PassportArgumentResolver`를 자동 등록한다. `META-INF/spring/org.springframework.boot.autoconfigure.AutoConfiguration.imports`에 선언되어 Spring Boot 3.x Auto-Configuration으로 동작한다.

## member-core 패키지 구조 (헥사고날)
...
### 6. 헥사고날 아키텍처 (member-core)

`member-core`는 포트·어댑터 패턴을 따른다. 도메인과 유스케이스는 `member-core`가 정의하고, 어댑터(`member-infra`)가 포트를 구현한다. `SignupService`는 `@Component`가 아닌 일반 클래스이며, `auth-api`의 `ApplicationServiceConfig`에서 `@Bean`으로 등록한다.
...
> 정의: `services/libs/passport/src/main/java/com/econo/auth/passport/PassportException.java`
...
> 정의: `services/libs/member-core/src/main/java/com/econo/auth/member/exception/`
...
```
services/libs/passport/src/test/java/com/econo/auth/passport/
...
services/libs/member-core/src/test/java/com/econo/auth/member/
├── application/usecase/
│   └── SignupServiceTest.java     # SignupService 단위 테스트
└── domain/
    └── MemberTest.java           # Member 도메인 단위 테스트

services/libs/member-infra/src/test/java/com/econo/auth/member/infra/
└── adapter/out/
