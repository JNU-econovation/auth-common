...
├── docs/                             # 문서 (ARCHITECTURE.md, CONVENTION.md)
...
본 문서의 아키텍처 설계와 관련된 스킬을 아래에 정리한다. 스킬 파일 위치: `.claude/skills/{스킬명}/SKILL.md`

### 아키텍처 근거

| 스킬 | 경로 | 본 문서와의 관계 |
|------|------|-----------------|
| **hexagonal-architecture** | `hexagonal-architecture/SKILL.md` | core 계층(프레임워크 무의존 도메인) → web 계층(Spring MVC 어댑터) → config 계층(Composition Root) 구조의 이론적 근거. 포트/어댑터 원칙에 따라 `Passport`는 순수 도메인, `PassportArgumentResolver`는 인바운드 어댑터, `AuthAutoConfiguration`은 Composition Root 역할을 한다. |
| **springboot-patterns** | `springboot-patterns/SKILL.md` | auth-api 모듈의 Controller → Service → Repository 계층 구조, DTO 설계, 예외 처리, 캐싱, 비동기 처리 패턴의 근거 |
| **jpa-patterns** | `jpa-patterns/SKILL.md` | auth-infra 모듈의 Entity 설계, Repository 패턴, 트랜잭션 전략, N+1 방지의 근거 |

### 테스트/검증

| 스킬 | 경로 | 본 문서와의 관계 |
|------|------|-----------------|
| **springboot-tdd** | `springboot-tdd/SKILL.md` | 테스트 구조 섹션의 단위 테스트(도메인 로직, Spring 불필요) + 통합 테스트(`@SpringBootTest` + `MockMvc`) 전략의 근거 |
| **springboot-verification** | `springboot-verification/SKILL.md` | 빌드 → 정적 분석 → 테스트 → 보안 스캔 → 린트 → 디프 리뷰 6단계 검증 루프 |

### 보안

| 스킬 | 경로 | 본 문서와의 관계 |
|------|------|-----------------|
| **springboot-security** | `springboot-security/SKILL.md` | api-gateway의 JWT 필터, auth-api의 인증/인가 로직 설계 참조. auth-common-lib는 JWT를 직접 처리하지 않고 Gateway가 검증한 Passport를 수신한다. |
