...
## 동적 라우팅 등록 (client_credentials 클라이언트)

`client_credentials` 타입으로 클라이언트를 등록할 때 `upstreamUrl` + `pathPrefix`를 지정하면
`service_route` 테이블에 라우트가 자동 등록됩니다.

```bash
# 새 서비스 등록 + Gateway 라우팅 자동 추가
