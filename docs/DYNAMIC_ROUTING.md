...
| 등록 | 요청 | 업스트림 |
|------|------|----------|
| `pathPrefix=/api/eeos`, `upstreamUrl=https://host/api` | `/api/eeos/programs` | `https://host/api/programs` |
| | `/api/eeos` | `https://host/api` |
| `pathPrefix=/eeos`, `upstreamUrl=https://host` | `/eeos/api/programs` | `https://host/api/programs` |

`upstreamUrl`에 경로가 없으면(`https://host`) prefix만 제거되어 루트(`/`) 기준으로 전달된다. 내부적으로 라우트 URI는 호스트부만 쓰고 경로 결합은 `RewritePath` 필터로 처리한다(Spring Cloud Gateway가 라우트 URI의 경로를 무시하기 때문). 정적 보호 라우트는 auth-api가 전체 경로를 기대하므로 이 치환을 쓰지 않는다.
...
- 동적 라우트는 경로 치환(rewrite) 모델이다. `pathPrefix`를 `upstreamUrl`로 치환한다 — 예: `pathPrefix=/api/eeos`, `upstreamUrl=https://host/api`이면 `/api/eeos/users` 요청은 `https://host/api/users`로 전달된다(위 "경로 치환 모델" 표 참고)
